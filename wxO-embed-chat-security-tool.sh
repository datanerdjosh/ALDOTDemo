...
# Non-CPD pattern
if [[ "${FULL_INSTANCE_API_URL}" =~ ^(https?://[^/]+)/instances/([^/?#]+) ]]; then
  IS_CPD="false"
...
echo "Detected Platform: IBM Cloud / AWS"
echo
...
if [[ -z "${IAM_URL:-}" ]]; then
  ...
  case "${API_URL_NOAPI}" in
    ...
    https://us-east.watson-orchestrate.cloud.ibm.com)
      ...
      ;;
    *)
      echo -e "${YELLOW}⚠️  Unknown API host: ${API_URL_NOAPI}${NC}"
...
echo
echo -e "${GREEN}Security has been disabled. Your embedded chat should now work without authentication.${NC}"
